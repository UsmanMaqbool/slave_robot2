...
pc_fetcher::pc_fetcher(){
    _kfSub = _nh.subscribe("/lsd_slam_r2/graph", 10, &pc_fetcher::kfCallback, this);
    _pcSub = _nh.subscribe("/lsd_slam_r2/keyframes", 20, &pc_fetcher::pcCallback, this);
    _pcPub = _nh.advertise<sensor_msgs::PointCloud2>("/slave_robot2/pointcloud2", 5);
    _kFMsgStampedPub = _nh.advertise<slave_robot2::keyframeMsgStamped>("/slave_robot2/kFMsgStamped", 5);
    _kFGraphMsgStampedPub = _nh.advertise<slave_robot2::keyframeGraphMsgStamped>("/slave_robot2/kFGraphMsgStamped", 5);
}
...
void pc_fetcher::pcCallback(slave_robot2::keyframeMsgConstPtr msg){
...
void pc_fetcher::kfCallback(slave_robot2::keyframeGraphMsgConstPtr graph_msg) {
	kfIndex=(uint32_t)graph_msg->numFrames;

    this->copykFGraphMsgTokFGraphMsgStamped(graph_msg);
}

void pc_fetcher::readkFMsg(slave_robot2::keyframeMsgConstPtr msg) {
...
void pc_fetcher::fillCloudMsg() {
    ...
    this->_cloud2.width = this->_pointcloud.points.size();
}

void pc_fetcher::copykFMsgTokFMsgStamped(slave_robot2::keyframeMsgConstPtr msg) {
	ros::Time timeStampTmp(msg->time);
...
void pc_fetcher::copykFGraphMsgTokFGraphMsgStamped(slave_robot2::keyframeGraphMsgConstPtr graph_msg) {
	...
	this->_kFGraphMsgStamped.numFrames = graph_msg->numFrames;
	this->_kFGraphMsgStamped.frameData = graph_msg->frameData;
	this->_kFGraphMsgStamped.numConstraints = graph_msg->numConstraints;
	this->_kFGraphMsgStamped.constraintsData = graph_msg->constraintsData;
}
